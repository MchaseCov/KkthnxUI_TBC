...
local replaceBubbleFont = true
local replaceNameplateFont = true
local replaceBlizzFonts = true
...
local function UpdateBlizzardFonts()
	...
	if replaceNameFont then
		_G.UNIT_NAME_FONT = NAMEFONT
	end

	if replaceCombatFont then
	...
	if replaceBlizzFonts then
		...
		SetFont(_G.NumberFont_Outline_Huge, NUMBER, s and huge or 28, outline) -- 30
		...
		SetFont(_G.SystemFont_OutlineThick_Huge2, NORMAL, s and huge or 20, outline) -- 22
