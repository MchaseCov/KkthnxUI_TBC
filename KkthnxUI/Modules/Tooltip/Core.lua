...
function Module:GetUnit()
    ...
    if not unit then
        ...
        unit = mFocus and
                   (mFocus.unit or
                       (mFocus.GetAttribute and mFocus:GetAttribute("unit"))) or
                   "mouseover"
    end
...
function Module:GetLevelLine()
    ...
    for i = 2, self:NumLines() do
        local tiptext = _G["GameTooltipTextLeft" .. i]
        local linetext = tiptext:GetText()
        if linetext and string_find(linetext, LEVEL) then return tiptext end
    end
end
...
function Module:OnTooltipSetUnit()
    ...
    if UnitExists(unit) then
        ...
        if isPlayer then
            ...
            local status = (UnitIsAFK(unit) and AFK) or
                               (UnitIsDND(unit) and DND) or
                               (not UnitIsConnected(unit) and PLAYER_OFFLINE)
            if status then
            ...
            if guildName and hasText then
                ...
                if IsInGuild() and guildName == myGuild and guildRealm ==
                    myGuildRealm then
        ...
        if level then
            ...
            local textLevel = string_format("%s%s%s|r", K.RGBToHex(diff),
                                            boss or string_format("%d", level),
                                            classification[classify] or "")
            local tiptextLevel = Module.GetLevelLine(self)
            if tiptextLevel then
                local pvpFlag = isPlayer and UnitIsPVP(unit) and
                                    string_format(" |cffff0000%s|r", PVP) or ""
                ...
                tiptextLevel:SetFormattedText(("%s%s %s %s"), textLevel,
                                              pvpFlag, unitClass, (not alive and
                                                  "|cffCCCCCC" .. DEAD .. "|r" or
                                                  ""))
            end
        end
        ...
        if UnitExists(unit .. "target") then
            ...
            local tar = string_format("%s%s",
                                      (tarRicon and ICON_LIST[tarRicon] ..
                                          "10|t") or "",
                                      Module:GetTarget(unit .. "target"))
...
function Module:StatusBar_OnValueChanged(value)
    ...
    if not self.text then self.text = K.CreateFontString(self, 11, nil, "") end
...
function Module:ReskinStatusBar()
    if not self or self:IsForbidden() or not self.StatusBar then return end
    ...
    self.StatusBar:SetStatusBarTexture(K.GetTexture(C["UITextures"]
                                                        .TooltipTextures))
...
function Module:GameTooltip_ShowStatusBar()
    if not self or self:IsForbidden() or not self.statusBarPool then return end

    local bar = self.statusBarPool:GetNextActive()
    if (not bar or not bar.text) or bar.isStyled then return end
...
function Module:GameTooltip_ShowProgressBar()
    ...
    if (not bar or not bar.Bar) or bar.isStyled then return end
...
function Module:GameTooltip_SetDefaultAnchor(parent)
    ...
    if C["Tooltip"].Cursor then
        ...
        if not GameTooltip_Mover then
            GameTooltip_Mover = K.Mover(self, "Tooltip", "GameTooltip", {
                "BOTTOMRIGHT", UIParent, "BOTTOMRIGHT", -182, 36
            }, 240, 120)
        end
...
function Module:GameTooltip_ComparisonFix(anchorFrame, shoppingTooltip1,
                                          shoppingTooltip2, _,
                                          secondaryItemShown)
...
fakeBg:SetBackdrop({
    ...
    edgeFile = "Interface\\AddOns\\KkthnxUI\\Media\\Border\\" ..
        C["General"].BorderStyle.Value .. "\\Border_Tooltip.tga",
    edgeSize = 12,
...
local function __GetBackdropColor() return 0.04, 0.04, 0.04, 0.9 end

local function __GetBackdropBorderColor() return 1, 1, 1 end
...
function Module:ReskinTooltip()
    ...
    if not self.isTipStyled then
        ...
        if self.StatusBar then Module.ReskinStatusBar(self) end
    ...
    if C["General"].ColorTextures then
        self.tooltipStyle.KKUI_Border:SetVertexColor(unpack(C["General"]
                                                                .TexturesColor))
    else
    ...
    if C["Tooltip"].ClassColor and self.GetItem then
        ...
        if item then
            ...
            if color then
                self.tooltipStyle.KKUI_Border:SetVertexColor(color.r, color.g,
                                                             color.b)
            end
        end
    end
...
function Module:SharedTooltip_SetBackdropStyle()
    if not self.isTipStyled then return end
...
function Module:OnEnable()
    ...
    GameTooltip.StatusBar:SetScript("OnValueChanged",
                                    Module.StatusBar_OnValueChanged)
    hooksecurefunc("GameTooltip_ShowStatusBar", Module.GameTooltip_ShowStatusBar)
    hooksecurefunc("GameTooltip_ShowProgressBar",
                   Module.GameTooltip_ShowProgressBar)
    hooksecurefunc("GameTooltip_SetDefaultAnchor",
                   Module.GameTooltip_SetDefaultAnchor)
    hooksecurefunc("SharedTooltip_SetBackdropStyle",
                   Module.SharedTooltip_SetBackdropStyle)
    hooksecurefunc("GameTooltip_AnchorComparisonTooltips",
                   Module.GameTooltip_ComparisonFix)
...
function Module:RegisterTooltips(addon, func) tipTable[addon] = func end
...
Module:RegisterTooltips("KkthnxUI", function()
    local tooltips = {
        AutoCompleteBox, ChatMenu, EmbeddedItemTooltip, EmoteMenu,
        FriendsTooltip, GameTooltip, GeneralDockManagerOverflowButtonList,
        IMECandidatesFrame, ItemRefShoppingTooltip1, ItemRefShoppingTooltip2,
        ItemRefTooltip, LanguageMenu, NamePlateTooltip, ShoppingTooltip1,
        ShoppingTooltip2, VoiceMacroMenu, WorldMapTooltip
    }
    ...
    local function reskinDropdown()
        for _, name in pairs({
            "DropDownList", "L_DropDownList", "Lib_DropDownList"
        }) do
    ...
    C_Timer_After(5, function()
        ...
        -- Lib minimap icon
        if LibDBIconTooltip then Module.ReskinTooltip(LibDBIconTooltip) end

        -- TomTom
        if TomTomTooltip then Module.ReskinTooltip(TomTomTooltip) end

        -- RareScanner
        if RSMapItemToolTip then Module.ReskinTooltip(RSMapItemToolTip) end

        if LootBarToolTip then Module.ReskinTooltip(LootBarToolTip) end

        -- Narcissus
        if NarciGameTooltip then Module.ReskinTooltip(NarciGameTooltip) end

        if CharNoteTooltip then Module.ReskinTooltip(CharNoteTooltip) end

        if AceGUITooltip then Module.ReskinTooltip(AceGUITooltip) end
        ...
        if GameCooltipFrame then Module.ReskinTooltip(GameCooltipFrame) end
    end)
end)
...
Module:RegisterTooltips("Blizzard_EventTrace",
                        function() Module.ReskinTooltip(EventTraceTooltip) end)
